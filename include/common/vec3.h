...
template <typename T> inline const T &TVec3<T>::operator[](int n) const
{
	assert(n >= 0 && n <= 2);
	return (&x)[n];
}

template <typename T> inline T &TVec3<T>::operator[](int n)
{
	assert(n >= 0 && n <= 2);
	return (&x)[n];
}

template <typename T> inline bool TVec3<T>::operator==(const TVec3<T> &a) const
{
	return (x == a.x && y == a.y && z == a.z);
}

template <typename T> inline TVec3<T> TVec3<T>::operator-() const
{
	return { -x, -y, -z };
}
...
template <typename T> inline T norm2(const TVec3<T> a)
{
